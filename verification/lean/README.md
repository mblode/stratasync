...
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DeltaPipeline` | `client/src/sync/{delta-pipeline,cursor}.ts`, `core/src/sync/{sync-id,delta-applier}.ts`                                                                    | Sync-id comparison equals numeric order; deltas apply exactly once, in order, under any mix of sources; the cursor is monotone and crash-safe, and never passes the identity maps (a failed packet is redelivered); stale catch-up pages, stream restarts and re-bootstrap resumes never cross runs; updates, archives and unarchives never create stub rows for rows not stored locally |
| `Outbox`        | `client/src/outbox-manager.ts`, `storage-idb/src/stores/outbox.ts`                                                                                          | Deliveries are a prefix of the queue after transport failures; no transaction is sent twice; server dedup applies each once; rollback uses the latest rebased original; replay follows batchIndex even if the clock steps back; a transaction is reported rejected iff the server never applies it                                                                                       |
...
| `Server`        | `server/src/**`                                                                                                                                             | AsyncMutex exclusion and FIFO; subscribe/replay/live handoff delivers exactly C+1..n; publishes arriving out of order across processes are gap-filled so no committed id is skipped; concurrent duplicate submissions dedupe; stale cursors are detected even after retention empties the table; keyset pagination (including NULL cursor fields) never skips or repeats                 |
