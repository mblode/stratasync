...
| `@Reference`      | Model A -> Model B   | Many-to-one  | Yes (on A)                |
...
| `@OneToMany`      | Alias for @OneToMany | One-to-many  | No                        |
...
## @Reference: explicit many-to-one
...
export class Comment extends Model {
  ...
  @Reference("Task", "comments", { foreignKey: "taskId" })
  ...
  @Reference(() => User, "comments", { foreignKey: "authorId" })
...
Use `@Reference` for string-based references, `@Reference` for factory functions (helps with circular imports). Both produce identical metadata.
