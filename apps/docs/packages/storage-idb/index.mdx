...
The adapter derives a deterministic database name from the `userId`, `version`
and `userVersion` you pass to `createIndexedDbStorage`, so each user and version
combination gets its own database and switching accounts does not require
clearing data. The derivation itself is internal; set the inputs and the adapter
does the rest.
...
Bump `userVersion` when a local schema change should start a client from a
clean database.

## Public surface

The package exports the factory and two types:

```ts
import { createIndexedDbStorage } from "@stratasync/storage-idb";
import type { DatabaseInfo, PartialIndexEntry } from "@stratasync/storage-idb";
```

Everything else on this page is reached through the adapter instance
(`storage.get(...)`, `storage.setPartialIndex(...)`, and so on), not imported
directly. Store-name derivation and the per-store read and write helpers are
internal to the package and are not part of the public API.
