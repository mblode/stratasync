...
`@Reference` on the child creates the foreign key; `@OneToMany` on the parent creates the collection.
...
class Task extends Model {
  ...
  @Reference(() => Project, "tasks")
  project!: Project;
  // Creates the projectId foreign key, which @OneToMany reads back
}
```
