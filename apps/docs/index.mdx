...
description: Build local-first TypeScript, React, and Next.js apps with instant reads, offline writes, and real-time collaboration.
---
...
Reads come from a local store, so the UI stays fast. Writes queue offline and sync when you reconnect. A server-sequenced log ensures every client converges.
