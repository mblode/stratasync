...
`SystemSyncRuntime` owns a scheduler; close it when discarding its engine. Tests use an injected clock and seeded IDs. The optional `canReplayAbsentInsert` host policy may reauthorize new inserts against fresh `Bootstrap.authorizedGroups`; the default withholds every absent target. Hosts must prove all relevant parent scopes before opting in.

`refresh()` supports foreground REST polling through the same serialized catch-up path. The host observes `rows()`, `snapshot()`, `state` and `lastError`; a lifecycle-aware Flow adapter is still required for Compose.
