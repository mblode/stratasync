...
- `notifyGroupsChanged(userId)` writes a `"G"` sync action addressed to that user's own group carrying their full current group list, then publishes it. It is an action rather than a frame so it is durable: a user who is offline still receives it on their next replay or catch-up. It does not write or revoke the membership itself — call it after you do.
