...
- `notifyGroupsChanged(userId)` writes a `"G"` sync action addressed to that user's own group carrying their full current group list, then publishes it. It is an action rather than a frame so it is durable: a user who is offline still receives it on their next replay or catch-up. WebSocket sessions immediately intersect their existing groups with this list, preventing later delivery from revoked groups without allowing additions before reauthorization. It does not write or revoke the membership itself — call it after you do.
