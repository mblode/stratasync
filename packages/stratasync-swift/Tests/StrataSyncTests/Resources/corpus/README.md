...
here, Swift and Kotlin in this repository (Done Bear temporarily consumes a frozen Swift snapshot)) and prose
...
Swift and Kotlin both run all eleven current shared engine scenarios and seven client wire-vector groups. Swift also runs its package-local resilience suite. Capability manifests describe tested shared coverage, not every feature implemented in each SDK; Android transport/storage adapters and membership-change reconciliation are not implemented. Client capability claims do not certify production readiness.
